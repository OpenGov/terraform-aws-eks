resource "aws_autoscaling_group" "workers" {
  ...
  enabled_metrics = compact(
    split(
      ",",
      coalesce(
        lookup(var.worker_groups[count.index], "asg_metrics", ""),
        local.workers_group_defaults["asg_metrics"],
      ),
    ),
  )
  ...
  tags = [
    concat(
      [
        ...
        {
          "key" = "k8s.io/cluster-autoscaler/node-template/label${lookup(
            var.worker_groups[count.index],
            "autoscaling_node_label_key",
            "",
            ) == "" ? "-autoscaling-to-zero-nodes-disabled" : "/${lookup(
              var.worker_groups[count.index],
              "autoscaling_node_label_key",
              "",
          )}"}"
          "value" = lookup(
            var.worker_groups[count.index],
            "autoscaling_node_label_value",
            "",
          )
          "propagate_at_launch" = lookup(
            var.worker_groups[count.index],
            "autoscaling_node_label_key",
            "",
          ) == "" ? false : true
